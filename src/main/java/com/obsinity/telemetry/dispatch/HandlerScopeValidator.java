...
/**
 * Startup validator that enforces safe combinations of @OnEvent, @OnEveryEvent, @OnUnMatchedEvent
 * with optional @EventScope on @TelemetryEventHandler components.
 */
@Component
public class HandlerScopeValidator implements SmartInitializingSingleton {
		...
		for (Map.Entry<String, Object> e : beans.entrySet()) {
			...
			if (hasComponentUnmatched && scope == null) {
				throw new BeanCreationException(beanName,
					err(userClass) + " mixes @OnUnMatchedEvent(scope=COMPONENT) and @OnEvent but has no @EventScope. " +
			...
			for (Method m : onEventMethods) {
				...
				validateOnEventAgainstScope(beanName, userClass, m, a,
					scopePrefixes, scopeLifecycles, scopeKinds, scopeErrorMode);
			}
			...
			for (Method m : userClass.getMethods()) {
				if (m.isAnnotationPresent(OnEvent.class) && m.isAnnotationPresent(OnUnMatchedEvent.class)) {
					throw new BeanCreationException(beanName,
						err(userClass, m) + "cannot have both @OnEvent and @OnUnMatchedEvent.");
				}
			}
			...
			for (Method m : methodsAnnotated(userClass, OnEveryEvent.class)) {
				...
				if (!scopeLifecycles.isEmpty() && ann.lifecycle().length > 0
				...
				if (!scopeKinds.isEmpty() && ann.kinds().length > 0
					&& !intersects(scopeKinds, Set.of(ann.kinds()))) {
					throw new BeanCreationException(beanName,
	...
	private static void validateOnEventAgainstScope(String beanName, Class<?> userClass, Method m, OnEvent a,
													Set<String> scopePrefixes, Set<Lifecycle> scopeLifecycles,
													Set<SpanKind> scopeKinds, EventScope.ErrorMode scopeErrorMode) {
		...
		if (!scopePrefixes.isEmpty()) {
			...
			if (!ok) {
				throw new BeanCreationException(beanName,
					err(userClass, m) + "event name outside component @EventScope prefixes. " +
						"Scope=" + scopePrefixes + ", method name='" + eventName + "'.");
			}
		}

		// Lifecycle intersection
		if (!scopeLifecycles.isEmpty() && a.lifecycle().length > 0
			&& !intersects(scopeLifecycles, Set.of(a.lifecycle()))) {
			throw new BeanCreationException(beanName,
				err(userClass, m) + "lifecycles do not intersect component @EventScope.");
		}

		// Kind intersection
		if (!scopeKinds.isEmpty() && a.kinds().length > 0
			&& !intersects(scopeKinds, Set.of(a.kinds()))) {
			throw new BeanCreationException(beanName,
				err(userClass, m) + "span kinds do not intersect component @EventScope.");
		}
