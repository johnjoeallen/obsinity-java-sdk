...
/**
 * Trimmed to validate only the multi-prefix (OR) scope behavior:
 *   @OnEventScope("orders.") OR @OnEventScope("payments.")
 *
 * All other scenarios (success/failure split, out-of-scope unmatched, etc.)
 * are now covered by TelemetryRulesCoverageTest.
 */
@TelemetryBootSuite(classes = EventScopeSelectionTest.Config.class)
class EventScopeSelectionTest {

	@Autowired Config.Flows flows;
	@Autowired OrdersOrPaymentsFinished ordersOrPaymentsFinished;
